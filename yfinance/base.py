...
class TickerBase():
    ...
    def _get_info_only(self, kind=None, proxy=None):
        """
        Subrotina para pegar somente info, sem o resto, fica mais rapido e da menos pau
        :param kind:
        :param proxy:
        :return:
        """
...
#        self._fundamentals = True
    ...
    def get_info(self, proxy=None, as_dict=False, *args, **kwargs):
        # modificado pra ficar mais rapido
        #self._get_fundamentals(proxy)
        self._get_info_only(proxy)
        data = self._info
        if as_dict:
            return data.to_dict()
        return data
